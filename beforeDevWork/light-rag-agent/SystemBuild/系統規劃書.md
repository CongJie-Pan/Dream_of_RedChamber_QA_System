...
2. **問題分析與處理**
   - 複雜度分析：系統評估問題的複雜度（簡單、中等、複雜）
   - 問題分解：對於複雜問題，系統將其分解為多個子問題
   - 查詢優化：根據問題特性優化檢索參數

3. **知識檢索流程**
   - 檢索策略選擇：根據問題類型選擇適當的檢索模式（混合、語義、向量）
   - 並行檢索：同時檢索多個子問題的相關資訊
   - 結果過濾：過濾低相關性的檢索結果，確保質量
...
4. **上下文構建**
   - 篩選：選擇最相關的檢索結果
   - 組織：將選定的檢索結果組織成連貫的上下文
   - 格式化：將上下文轉換為適合模型輸入的格式
