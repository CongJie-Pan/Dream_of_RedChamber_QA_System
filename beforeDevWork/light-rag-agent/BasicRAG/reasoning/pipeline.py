...
class ReasoningPipeline:
    ...
    def process_subproblem(self, subproblem: Dict[str, Any], original_query: str,
        ...
        # For BasicRAG, ensure retrieval_method is set to "vector"
        if "retrieval_method" in parameters and parameters["retrieval_method"] != "vector":
            logger.debug(f"Forcing retrieval_method to 'vector' for BasicRAG (was: {parameters['retrieval_method']})")
            parameters["retrieval_method"] = "vector"
        ...
        if self.rag_system:
            try:
                ...
                # Adapt this to match your RAG system's API - BasicRAG typically uses a vector database
                # Extract relevant parameters for the vector search
                top_k = parameters.get("top_k", 5)
                similarity_threshold = parameters.get("similarity_threshold", 0.6)
                
                # Call the RAG system's retrieval method
