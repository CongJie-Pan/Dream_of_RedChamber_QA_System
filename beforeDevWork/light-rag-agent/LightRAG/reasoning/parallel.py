"""
Parallel Processing Module for LightRAG reasoning system.
...
class ParallelProcessor:
    ...
    def process_by_levels(self, 
        ...
        for i, level in enumerate(levels):
            ...
            level_questions = [self.nodes.get(sq_id) for sq_id in level if sq_id in self.nodes]
...
class RetrievalOrchestrator:
    ...
    def optimize_execution_plan(self, 
        ...
        use_parallel = max_parallelism > 1 and len(sub_questions) > 2
        
        # If the query is very complex, we might want to enforce serialization
        # to ensure proper dependency handling
        if query_complexity == "complex" and len(sub_questions) > 5:
            # For very complex queries, we might want to be more cautious
            # and only parallelize when there's a clear benefit
            use_parallel = max_parallelism >= 3
