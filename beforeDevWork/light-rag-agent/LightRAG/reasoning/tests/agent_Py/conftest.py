"""
Pytest configuration for the agent.py module tests.

This module contains fixtures and configuration for pytest, including
error handling and logging mechanisms for test results.
"""
...
def get_test_result_file(nodeid):
    ...
    """
    # Extract the test module name from the nodeid (e.g., "test_agent.py" from "test_agent.py::TestReasoningAgent::test_init")
    parts = nodeid.split("::")
...
def mock_model():
    """
    Fixture that provides a mock DeepSeekModel for testing the ReasoningAgent.
    """
    class MockDeepSeekModel:
        ...
        def batch_call(self, prompts, options=None):
            ...
            self.calls.extend([{"prompt": p, "options": options} for p in prompts])
            return [f"Mock response for {i+1}" for i, _ in enumerate(prompts)]
            
        def get_call_history(self):
            """Helper method to retrieve the call history"""
            return self.calls
...
def mock_cot():
    """
    Fixture that provides a mock ChainOfThought for testing.
    """
    class MockChainOfThought:
        ...
        def decompose_question(self, question):
            """Mock method for question decomposition"""
            self.decompose_count += 1
            
            # Return mock sub-questions
            return [
                {
                    "id": 1,
                    "question": "What is X?",
                    "relevance": "Understanding X is important for the original question",
                    "dependencies": []
                },
                {
                    "id": 2,
                ...
                {
                    "id": 3,
                    "question": "What are the implications of Z?",
                    "relevance": "Z has important implications for the answer",
                    "dependencies": [1, 2]
                }
            ]
