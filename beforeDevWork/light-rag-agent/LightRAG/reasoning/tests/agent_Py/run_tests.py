#!/usr/bin/env python
"""
Script to run the agent.py module tests and ensure results are properly saved.

This script will:
1. Run all the tests in the agent_Py directory
...
def verify_test_results():
    ...
    # Find the test_agent directories
    agent_test_dirs = [d for d in subdirs if d.startswith("test_agent_")]
    
    if not agent_test_dirs:
        print(f"ERROR: No test_agent directories found in: {TEST_RESULTS_DIR}")
        return False
    
    # Check the most recent directory for test result files
    latest_dir = max(agent_test_dirs, key=lambda d: os.path.getmtime(os.path.join(TEST_RESULTS_DIR, d)))
    latest_dir_path = os.path.join(TEST_RESULTS_DIR, latest_dir)
    ...
    # Check for files matching expected test names
    expected_patterns = ["ReasoningStepLogger", "ReasoningAgent"]
    found_patterns = 0
    ...
    for pattern in expected_patterns:
        ...
        if matching_files:
            found_patterns += 1
            print(f"  - Found {len(matching_files)} files for {pattern}")
    ...
    if found_patterns == len(expected_patterns):
        print(f"SUCCESS: Found files for all expected test patterns")
        return True
    else:
        print(f"WARNING: Only found files for {found_patterns} of {len(expected_patterns)} expected patterns")
        return False

def main():
    """Main function to run the tests."""
    print("=" * 80)
    print(f"Running agent.py module tests")
