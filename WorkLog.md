...
### 2025/5/4
- 今天主要是先建立reasoning module 的每個單元測試檔案，但因為太耗時間了，所以就想先做計畫書。
- 單元測試可以先放一放，咱們先把計劃書完成，尤其是做reason模塊的專題計劃書。計劃書完成後，要做出簡體中文和繁體中文兩個版本。簡體中文版本可以先給兆基看，問他覺得哪些地方還能修正，因為之後想拿給王軍老師看看，問問這樣做行不行。繁體中文版本是給錦慧老師看的。
- 另外，關于單元測試部分，我在想能不能自動進行，我還不確定Cursor有沒有這個功能，或者能不能通過MCP這種方式實現。大概就是這些。
